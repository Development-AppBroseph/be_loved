...
class GalleryPage extends StatefulWidget {
  ...
  GalleryPage(
      ...
      required this.hideGalleryFileID,
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<GalleryBloc, GalleryState>(
      ...
      builder: (context, state) {
        if (state is GalleryFilesInitialState ||
            state is GalleryFilesLoadingState) {
          return Container();
        }
        return SingleChildScrollView(
          child: Column(
            children: bloc.groupedFiles
                .map((group) => Stack(
                      children: [
                        Wrap(
                          spacing: 4.w,
                          ...
                          children: [
                            ...
                            ...List.generate(
                                ...
                                (index) => MiniMediaCard(
                                      ...
                                      isSelected: widget.deletingIds.contains(
                                          group.additionalFiles[index].id),
                                      onLongTap: () {
                                      ...
                                      onTap: () {
                                        if (widget.deletingIds.isNotEmpty ||
                                            widget.isForSelecting) {
                                          widget.onSelectForDeleting(
                                            group.additionalFiles[index].id,
                                          );
                                        } else {
                                          if (group
                                              .additionalFiles[index].isVideo) {
                                            Navigator.push(
                                                context,
                                                CupertinoPageRoute(
                                                  builder:
                        ...
                        if (group.additionalFiles.length -
                                    galleryGroupingCount(group) !=
                                0 &&
                            !widget.isForSelecting)
                          Positioned(
                              ...
                              child: BackdropFilter(
                                ...
                                child: Container(
                                    padding: EdgeInsets.symmetric(
                                        vertical: 4.h, horizontal: 11.w),
                                    color: Colors.white.withOpacity(0.7),
                                    alignment: Alignment.center,
                                    child: Text(
                                      '+${group.additionalFiles.length - galleryGroupingCount(group)}',
      ...
      Future.delayed(Duration(milliseconds: 150), () {
        ...
        for (var gItem in bloc.groupedFiles) {
          if (gItem.mainPhoto.id == file.id) {
            bloc.groupedFiles[bloc.groupedFiles.indexOf(gItem)].topPosition =
                getWidgetPosition(mainKey).dy +
                    ((widget.position == 0) ? 0 : (widget.position - 10.h));
            print(
                'SETTING: ${bloc.groupedFiles[bloc.groupedFiles.indexOf(gItem)].topPosition}');
            break;
          }
        }
      });
        ...
        if (widget.deletingIds.isEmpty && !widget.isForSelecting) {
          Navigator.of(context).push(PageRouteBuilder(
            pageBuilder: (_, __, ___) =>
                file.isVideo && file.id != group.mainPhoto.id
                    ? VideoView(
                        url: file.urlToFile,
